...
import { DefectEntry, UnitDateEntry, SmuDocShiftEntry, ExtractionStatus, HistoryEntry } from './types';
...
import { AlertCircle, Info, History, Trash2, FileText, Clock, Table as TableIcon, LayoutDashboard } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    try {
      const savedHistory = localStorage.getItem('mining_defect_history');
      if (savedHistory) {
  ...
  useEffect(() => {
    try {
      // Strip large fileData before saving to localStorage to avoid quota limits
      const persistentHistory = history.map(({ fileData, ...rest }) => rest);
      localStorage.setItem('mining_defect_history', JSON.stringify(persistentHistory));
    } catch (e) {
  ...
  const addToHistory = (
    ...
    data: string, 
    defects: DefectEntry[] = [], 
    unitDates: UnitDateEntry[] = [], 
    smus: SmuDocShiftEntry[] = []
  ) => {
  ...
  const loadFromHistory = (entry: HistoryEntry) => {
    ...
    setDefectData(entry.defectData);
    setUnitDateData(entry.unitDateData);
    setSmuData(entry.smuData);
    setStatus('success');
    setError(null);
  };
  ...
  const unreliableCount = defectData.filter(d => d.cek === 'Kurang Jelas' || d.cek === 'Tidak Jelas').length;
  ...
  const processDefects = async () => {
    ...
    try {
      ...
      setStatus('success');
      addToHistory(fileName!, mimeType, fileData, extractedData, [], []);
    } catch (err: any) {
      console.error(err);
  ...
  const exportDefectsToExcel = () => {
    const worksheetData = defectData.map(item => ({
      'Hal': item.halaman, 'No Unit': item.equipment_number, 'Category': item.id_defect, 'Date': item.date,
      'crew': '', 'Shift': item.shift, 'Distance': item.distance, 'finish': 0, 'OP': 'L',
      'Description': item.deskripsi_defect, 'Inspected': item.inspected_by, 'Status Cek': item.cek
    }));
    const worksheet = XLSX.utils.json_to_sheet(worksheetData);
    worksheet['!cols'] = [
      { wch: 10 }, { wch: 20 }, { wch: 10 }, { wch: 15 }, { wch: 10 }, { wch: 10 }, 
      { wch: 15 }, { wch: 10 }, { wch: 10 }, { wch: 50 }, { wch: 20 }, { wch: 15 }
    ];
  ...
  const exportUnitDateToExcel = () => {
    const worksheetData = unitDateData.map(item => ({
      'Hal': item.halaman, 'Unit Number': item.equipment_number, 'Date': item.date,
    }));
    const worksheet = XLSX.utils.json_to_sheet(worksheetData);
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img src="/logo.png" alt="Logo" className="w-10 h-10 rounded-lg object-cover" referrerPolicy="no-referrer" />
            <div>
              ...
              <div className="relative rounded-xl overflow-hidden border border-slate-200 group bg-slate-50 min-h-[300px] flex items-center justify-center">
                {!fileData ? (
                  <div className="text-center p-8">
                    <div className="w-20 h-20 bg-slate-100 text-slate-400 rounded-2xl flex items-center justify-center mx-auto mb-4">
                      <FileText className="h-10 w-10" />
                    </div>
                    <p className="font-bold text-slate-800 break-all">{fileName}</p>
                    <p className="text-slate-500 text-sm mt-2">Preview unavailable for history items to save space.</p>
                  </div>
                ) : isPDF ? (
                  <div className="text-center p-8">
              ...
              <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {history.map(entry => (
                  <div 
                    ...
                    className="p-3 rounded-xl border border-slate-100 hover:border-blue-200 hover:bg-blue-50 transition-all cursor-pointer group"
                  >
                    <div className="flex items-start gap-3">
                      <div className="w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center text-slate-500 group-hover:bg-blue-100 group-hover:text-blue-600 shrink-0">
                        <FileText className="h-4 w-4" />
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium text-slate-700 truncate group-hover:text-blue-700">{entry.fileName}</p>
                        ...
                        <div className="flex gap-1 mt-2">
                          {entry.defectData.length > 0 && <span className="px-1.5 py-0.5 bg-blue-100 text-blue-600 rounded text-[9px] font-bold">DEFECTS</span>}
                          {entry.unitDateData.length > 0 && <span className="px-1.5 py-0.5 bg-green-100 text-green-600 rounded text-[9px] font-bold">UNIT/DATE</span>}
                          {entry.smuData.length > 0 && <span className="px-1.5 py-0.5 bg-purple-100 text-purple-600 rounded text-[9px] font-bold">SMU/DOC</span>}
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          {status === 'success' && (defectData.length > 0 || unitDateData.length > 0 || smuData.length > 0) && (
            <div className="space-y-6">
                  ...
                  {defectData.length > 0 && (
                    <div className="flex bg-slate-100 p-1 rounded-lg">
                      <button 
                        ...
                        className={`px-3 py-1.5 rounded-md text-xs font-bold flex items-center gap-2 transition-all ${activeTab === 'table' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                      >
                      ...
                      <button 
                        ...
                        className={`px-3 py-1.5 rounded-md text-xs font-bold flex items-center gap-2 transition-all ${activeTab === 'dashboard' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                      >
              ...
              {defectData.length > 0 && activeTab === 'dashboard' ? (
                <DefectDashboard data={defectData} />
              ) : (
                <>
                  {unreliableCount > 0 && (
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-amber-700 mt-1">
                          Found {unreliableCount} entries with low visual clarity. These are highlighted in the table below. Please verify them against the original document.
                        </p>
                      </div>
                    </div>
                  )}
